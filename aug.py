...
transform = transforms.Compose([
    ...
    transforms.RandomApply([transforms.RandomRotation(1)], p=0.5),
    transforms.RandomApply([transforms.RandomCrop(4, padding=2)], p=0.15),
    transforms.RandomApply([transforms.ColorJitter(brightness=0.2, contrast=0.15, saturation=0.125, hue=0.125)], p=0.15),
    transforms.RandomApply([transforms.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.9, 1.1), shear=5)], p=0.25),
    transforms.RandomApply([transforms.RandomPerspective(distortion_scale=0.125, p=0.25)], p=0.125),
    transforms.ToTensor(),
